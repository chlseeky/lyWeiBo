...
#define IWUserFile [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"user.data"]
...
+ (void)userInfoDidsuccess:(void (^)(IWUser *))success failure:(void (^)(NSError *))failure
{
    ...
    [IWHttpTool get:@"https://api.weibo.com/2/users/show.json" parameters:param.mj_keyValues success:^(id responseObject) {
        MLOG(@"%@",responseObject);
...
+ (void)unreadCountDidsuccess:(void (^)(IWUserUnreadResult *))success failure:(void (^)(NSError *))failure
{
    ...
    [IWHttpTool get:@"https://rm.api.weibo.com/2/remind/unread_count.json" parameters:param.mj_keyValues success:^(id responseObject) {
        IWUserUnreadResult *userUnread = [IWUserUnreadResult mj_objectWithKeyValues:responseObject];
        if (success) {
            success(userUnread);
        }
