...
+ (void)composeWithImage:(UIImage *)image status:(NSString *)status success:(void (^)())success failure:(void (^)(NSError *))failure
{
    ...
    // 发送请求
    [IWHttpTool upload:@"https://api.weibo.com/2/statuses/update.json" parameters:param.mj_keyValues uploadParam:uploadParam success:^{
        if (success) {
            success();
        }
...
+ (void)composeWithSatausText:(NSString *)status success:(void (^)())success failure:(void (^)(NSError *))failure
{
    ...
    // 发送请求
    [IWHttpTool post:@"https://api.weibo.com/2/statuses/update.json" parameters:param.mj_keyValues success:^(id responseObject) {
